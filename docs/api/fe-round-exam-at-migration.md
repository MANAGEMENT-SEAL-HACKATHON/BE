...
**Tự động:** lúc start app, `RoundExamAtSchemaMigration` backfill `exam_at` rồi `NOT NULL` (sau Hibernate thêm cột nullable).

**Lỗi thường gặp:** `Incorrect datetime value: '0000-00-00'` — do thêm `exam_at NOT NULL` trực tiếp lên bảng có dữ liệu cũ. Cách xử lý:

1. Chạy script thủ công: `src/main/resources/db/manual/V20260522_round_exam_at.sql`, **hoặc**
2. Xóa cột `exam_at` lỗi (nếu đã tạo dở): `ALTER TABLE rounds DROP COLUMN exam_at;` rồi **restart** app (BE mới).
