...
| BR-REG-029 | FR-10 | Registration | Invariant | isRegistrationClosed = closedEarlyAt != null OR now.isAfter(registrationEnd) (LocalDateTime) | đọc support | true/false | N/A | unit test giờ kết thúc trong cùng ngày | Implemented | hackathons/support/HackathonRegistrationSupport.java |
...
| BR-TRACK-008 | FR-03 | Track | Validation | Topic nhập/sửa ở GĐ1; không đổi sau KICKOFF hoặc sau khi đã bốc thăm | topic changed và (đã KICKOFF hoặc đã lottery) | Reject | INVALID_STATE | TC topic after kickoff/lottery | Implemented | tracks/service/impl/TrackServiceImpl.java |
...
| BR-TEAM-042 | FR-11/12 | Team | Gate | Leader phải có `hackathon_registrations` trước createTeam / inviteMember | chưa đăng ký | 422 | LEADER_NOT_REGISTERED | create without register | Implemented | TeamServiceImpl.java |
| BR-TEAM-043 | FR-12 | Team | Validation | Invitee phải registered cùng hackathon; lookup chỉ trả registered | chưa/khác sự kiện | 422 | INVITEE_NOT_REGISTERED | invite cross-event | Implemented | TeamServiceImpl.java / UserLookupServiceImpl.java |
| BR-TEAM-044 | FR-12 | Team | Gate | ACCEPT invite bị reject nếu invitee đã hủy / mất registration | không còn registration | 422 | INVITEE_NOT_REGISTERED | accept after unregister | Implemented | TeamServiceImpl.java#patchTeamMember |
...
| BR-SUB-018A | FR-16/FR-23 | Submission | Gate | Đã quay số thuyết trình thì không nộp muộn | afterDeadline && presentationShuffled | Từ chối | SUBMISSION_LOCKED_AFTER_SHUFFLE | Nộp muộn sau shuffle | Implemented | SubmissionServiceImpl |
...
| BR-RPROG-005 | FR-16A/FR-23 | RoundProgression | Gate | Đã shuffle → không duyệt bài muộn (không còn append queue) | presentationShuffled | Từ chối | SUBMISSION_LOCKED_AFTER_SHUFFLE | Review late sau shuffle | Implemented | SubmissionServiceImpl#reviewLate |
...
| BR-RPROG-015A | FR-16 | RoundProgression | Gate | Mọi đội eligible phải đã nộp (SL: ACTIVE+TRT; CK: TRP/ADVANCED) | submitted&lt;eligible | Từ chối | TEAMS_NOT_ALL_SUBMITTED | Close khi còn đội chưa nộp | Implemented | RoundProgressionServiceImpl#closeSubmissionEarly |
| BR-RPROG-015 | FR-16 | RoundProgression | SideEffect | Set closedEarlyAt; clamp deadline=now-5s; normalize submissionOpen≤deadline; notify SUBMISSION_CLOSED_EARLY | Close OK | Clamp deadline → mọi submit sau = late; notify SV/judge/mentor | N/A | Submit ngay sau close | Implemented | RoundProgressionServiceImpl#closeSubmissionEarly |
...
| BR-RPROG-026A | FR-22B/FR-24 | RoundProgression | Gate | Publish: autoApply waterfall rồi chặn nếu còn manual tiebreak | requiresManualReorder | Từ chối | TIEBREAK_REQUIRED | Publish khi còn hòa sâu | Implemented | RoundProgressionServiceImpl#publish |
...
| BR-RPROG-033 | FR-22B | RoundProgression | Policy | Waterfall: tiêu chí phụ → submitted_at → manual; chỉ MANUAL cần Coord | candidates | resolvedTier + suggestedOrderedTeamIds | N/A | Hòa total, khác submittedAt → SUBMISSION_TIME | Implemented | rounds/support/TiebreakRuleOrdering.java |
| BR-RPROG-034 | FR-22B | RoundProgression | SideEffect | Sau lock: autoApplyResolvableTiebreaks cho item không manual (Tầng 1/2) | Lock scoring | Ghi penalty 0.01 + notes lý do tầng | N/A | Lock với Tầng 1 tách được | Implemented | RoundProgressionServiceImpl#autoApplyResolvableTiebreaks;#applyTiebreakOrder |
...
| BR-RPROG-051 | FR-22B/FR-30 | RoundProgression | Gate | Còn **manual** unresolved tiebreak → không advance (auto Tầng 1/2 không chặn) | requiresManualReorder=true | Từ chối kèm unresolvedItems | TIEBREAK_REQUIRED | Advance khi còn hòa sâu | Implemented | RoundProgressionServiceImpl#advanceTeams |
...
> **Đã gỡ khỏi codebase (2026-07-31).** Module `appeals`, cửa sổ khiếu nại, T-5 delay, TeamReinstatement, và advance gate theo PENDING/UNDER_REVIEW **không còn**. Publish SL → chốt CK liền mạch sau khi xử lý đồng điểm. Script dọn DB tay: `db/manual/V20260801_drop_appeals.sql`. Các BR-APPEAL-001…008 dưới đây **Deprecated**.

| Rule ID | Status | Note |
|---------|--------|------|
| BR-APPEAL-001 … BR-APPEAL-008 | **Deprecated / Removed** | Xem git history nếu cần tham chiếu cũ |

## Tie-break Waterfall

| Rule ID | Related Req ID | Module | Rule Type | Business Rule Statement | Condition / Trigger | System Action / Expected Result | Exception / Error Message | Test Case / Example Data | Status | Evidence Link / Note |
|---------|-----------------|--------|-----------|-------------------------|---------------------|---------------------------------|---------------------------|--------------------------|--------|----------------------|
| BR-TIE-001 | FR-22B | Criteria | Invariant | Mỗi track / final round chỉ **đúng 1** tiêu chí `is_tiebreaker_priority=true`; không gắn cờ lên PENALTY | Tạo/sửa criterion thứ 2 có cờ / PENALTY+cờ | 422 | TIEBREAKER_PRIORITY_ALREADY_EXISTS\|TIEBREAKER_PRIORITY_PENALTY_NOT_ALLOWED | 2 criterion cùng cờ | Implemented | CriteriaServiceImpl |
| BR-TIE-002 | FR-22B | RoundProgression | Policy | Phân xử đồng điểm 3 tầng: (1) điểm tiêu chí phụ DESC → (2) on-time rồi `submitted_at` ASC → (3) manual | Cùng totalScore tại cutoff | Auto Tầng 1/2; Tầng 3 `requiresManualReorder` | N/A | Hòa total, khác điểm tiêu chí phụ | Implemented | TiebreakRuleOrdering#resolveWaterfall;RoundProgressionServiceImpl#enrichTiebreakItems |
| BR-TIE-003 | FR-22B | RoundProgression | Validation | Resolve thủ công bắt buộc `note` không rỗng; từ chối nếu nhóm đã auto-resolve được | note blank / tier≠MANUAL | 422 | VALIDATION_FAILED\|TIEBREAK_ALREADY_RESOLVED | Resolve khi đã tách bằng submittedAt | Implemented | ResolveTiebreakRequest;RoundProgressionServiceImpl#resolveTiebreak |
| BR-TIE-004 | FR-22B | RoundProgression | Gate | Advance chỉ chặn khi còn item **requiresManualReorder=true** (không chặn nhóm đã auto) | Manual unresolved | 422 TIEBREAK_REQUIRED | TIEBREAK_REQUIRED | Advance sau auto Tầng 1 | Implemented | RoundProgressionServiceImpl#advanceTeams |
| BR-TIE-005 | FR-22B/FR-30 | RoundProgression | Policy | reasonLabel vào CK = `Top {rank} — {bảng}`; không fallback `Top N` / `Top {topN}` khi thiếu rank | Advance roster / preview | Gán rank trong bảng theo điểm nếu thiếu | N/A | 2 đội cùng bảng → Top 1 và Top 2 | Implemented | RoundProgressionServiceImpl#buildAdvanceRosterItems |
...
| BR-EXPORT-008 | FR-34 | Export | Calculation | CSV_RANKINGS: ranking mọi round + TEAM_OTHER; BOM UTF-8; enrich judge_count (batch ScoreRepository), chapter_code/name, leader, members, DQ, submitted_at, is_late | type=CSV_RANKINGS | Xuất CSV rankings enriched | N/A | So header ExportCsvBuilder.HEADER_CSV_RANKINGS | Implemented | export_jobs/support/ExportCsvBuilder.java |
...
| BR-EXPORT-010 | FR-34 | Export | Calculation | FULL_REPORT: TEAMS, TEAM_MEMBERS, CRITERIA, JUDGE_ASSIGNMENTS, SUBMISSIONS, RANKINGS, CHAPTER/INDIVIDUAL_RANKINGS, PRIZES, APPEALS + SCORES_ANONYMIZED + RBL long + variance; CSV_SCORES enriched (team/judge/criterion/weighted) | type=FULL_REPORT / CSV_SCORES | CSV multi-section / scores | N/A | SECTION markers + HEADER_CSV_SCORES | Implemented | export_jobs/support/ExportCsvBuilder.java |
