...
| Hạng mục | Nội dung |
|----------|----------|
| Phạm vi | Tạo/cấu hình hackathon: vòng, bảng, tiêu chí, nhân sự, lịch sự kiện, readiness, kích hoạt ONGOING |
...
| Seeder | Vai trò |
|--------|---------|
| `DataInitializer` | Orchestrate 9 happy slugs |
...
Restart BE → `repairForFeTesting()` sync lịch; không hard-code ngày.
...
| Tabs | **Cấu hình chung**, **Vòng thi**, **Bảng đấu**, **Tiêu chí đánh giá**, **Nhân sự**, **Lịch trình & Sự kiện** |
| Kích hoạt | Header: **Xác nhận Kích hoạt** |
| Archive SV | `/student/results` |

---

## 6. Happy path (G1-H01 … G1-H08)
...
| G1-H02 | Happy | Coord | `setup?tab=rounds` | **Thêm vòng thi** ×2: Sơ loại + Chung kết → **Lưu** | 2 vòng hiển thị; CK shell có criteria placeholder | — | `RoundsTab` | `RoundController` |
| G1-H03 | Happy | Coord | `setup?tab=tracks` | **Thêm bảng** + **Upload PDF** đề SL | Track list; PDF icon/link | — | `TracksTab` | `TrackController` |
| G1-H04 | Happy | Coord | `setup?tab=criteria` | Thêm tiêu chí; Collapse mô tả; tổng **trọng số = 1.0** | Tag xanh 100%; **Lưu** OK | — | `CriteriaTab` | `CriteriaController` |
| G1-H05 | Happy | Coord | `setup?tab=people` | Gán Judge **INTERNAL** + Mentor vào track SL | Bảng nhân sự cập nhật | — | `PeopleTab` | `JudgeAssignmentController` |
| G1-H06 | Happy | Coord | `setup?tab=events` | **Thêm** KICKOFF → WORKSHOP → (khuyến nghị) AWARDS | Timeline 3 milestone; POST order đúng | — | `EventsTab` | `EventController` |
...
**Mode A shortcut:** G1-H02…H07 trên `seal-e2e-2026` — verify tab đã seed, chỉ demo H07 nếu cần.
...
## 8. Bad path (G1-B01 … G1-B05)

| ID | Loại | Role | URL | Thao tác UI | Kết quả UI | ErrorCode | FE | BE |
|----|------|------|-----|-------------|------------|-----------|----|----|
| G1-B01 | Bad | Coord | `setup?tab=events` | POST WORKSHOP khi chưa KICKOFF | Toast 422 | `EVENT_KICKOFF_MISSING` | `EventsTab` | `EventController` |
| G1-B02 | Bad | Coord | `setup?tab=events` | POST AWARDS khi chỉ có KICKOFF | Toast 422 | `EVENT_ORDER_VIOLATION` | `EventsTab` | `EventController` |
| G1-B03 | Bad | Coord | Setup header | Thiếu KICKOFF → click Kích hoạt | Nút **disabled** + tooltip blockers | `READINESS_INCOMPLETE` | `HackathonSetupPage` | `ReadinessService` |
| G1-B04 | Bad | Coord | `setup?tab=criteria` | Tổng weight ≠ 1.0 → Lưu | Validation đỏ / không pass readiness | — | `CriteriaTab` | `CriteriaController` |
| G1-B05 | Bad | Coord | `setup?tab=rounds` | Xóa vòng CK → readiness | Blocker `MISSING_FINAL_ROUND` | `MISSING_FINAL_ROUND` | `RoundsTab` | `ReadinessService` |
...
|----|------|------|-----|---------------------|------------------------|-----------|----|----|
| G1-S01 | Sabotage | Coord | Events | Tạo AWARDS trước WORKSHOP | 422 toast | `EVENT_ORDER_VIOLATION` | `EventsTab` | `EventController` |
| G1-S02 | Sabotage | Coord | Events | WORKSHOP cùng ngày KICKOFF | 422 | `EVENT_ORDER_VIOLATION` | `EventsTab` | `EventController` |
| G1-S03 | Sabotage | Coord | Rounds | Xóa Final Round → activate | Disabled / 422 | `MISSING_FINAL_ROUND` | `RoundsTab` | `HackathonStatusController` |
| G1-S04 | Sabotage | Coord | Setup | Activate khi DRAFT thiếu track | Disabled + tooltip | `READINESS_INCOMPLETE` | `HackathonSetupPage` | `ReadinessService` |
| G1-S05 | Sabotage | Coord | People | Gán **guest** vào track SL | Toast reject | `EXTERNAL_JUDGE_NOT_ALLOWED_IN_PRELIM` | `PeopleTab` | `JudgeAssignmentController` |
| G1-S06 | Sabotage | Coord | People | Gán FINAL_EXTERNAL ở GĐ1 | Toast reject | `JUDGE_FINAL_AT_PHASE1` | `PeopleTab` | `JudgeAssignmentController` |
| G1-S07 | Sabotage | Coord | Rounds | Xóa CK rồi cố publish round | Gate fail | `MISSING_FINAL_ROUND` | `RoundsTab` | `RoundController` |
...
| FE Events | `features/events/` |
| FE Rounds/Tracks/Criteria | `features/rounds/`, `features/tracks/`, `features/criteria/` |
| FE People | `features/people/` |
| BE Hackathon | `HackathonController`, `HackathonStatusController` |
| BE Readiness | `ReadinessService` / `GET .../readiness?target=ONGOING` |
| BE Events | `EventController` |
...
| Readiness API? | `gd1-full-test-matrix-and-seeds.md` §1.2 Gate G1–G5 |
