...
| **Resend (gia hạn)** | Chỉ khi invitation **đã hết hạn**, chưa `accepted_at`. Phải còn **≥ 48h trước KICKOFF** và KICKOFF **chưa bắt đầu**. Coordinator **tự kiểm tra còn slot judge** (ngoài hệ thống); nếu còn slot → `POST /invitations/{id}/resend` → MK tạm mới + **+72h**. Mentor/judge (nội bộ) được gán là **mặc định tham gia** — không có từ chối. Guest judge bấm link email / đổi MK = chấp nhận; nếu cần thay người thì coordinator **revoke** lời mời hoặc DELETE phân công rồi mời/gán người khác — **không** resend khi hết slot. | `GuestJudgeLifecycleService.assertResendAllowed`, `InvitationServiceImpl.resend` |
