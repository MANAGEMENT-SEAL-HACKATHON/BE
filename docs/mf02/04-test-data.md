...
### 1.3 Register tối giản (email + password + confirmPassword)
...
```json
{
  ...
  "confirmPassword": "Student@qa123"
}
```

**Kỳ vọng:** `201`, `data.status` = `"PENDING"`.
...
| `confirmPassword` khác `password` | 422 | `VALIDATION_FAILED` |

---

### 1.4 Hoàn thiện hồ sơ sau login (`PATCH /users/me`)

Sau khi register, user có thể login ngay (status vẫn `PENDING`) để bổ sung hồ sơ.
...
### 1.5 Login student — trước khi Coordinator duyệt
...
**Kỳ vọng:** `200` cho STUDENT `PENDING` để hoàn thiện hồ sơ.
...
### 3.2 PATCH profile (điền hồ sơ xét duyệt)
...
Form-data:
- key: `file`
- value: file ảnh thẻ (`.jpg/.jpeg/.png/.webp`, tối đa 5MB)

### 3.4 Xem ảnh thẻ đã upload
...
| `GET /api/v1/users/me` | Student PENDING (chưa duyệt) | `200` |
...
2. POST /auth/register       (email/password/confirmPassword)
3. POST /auth/login          (student PENDING)
4. PATCH /users/me           (fullName, userType, studentCode, chapter/institution)
5. POST /users/me/student-card
6. Coordinator login → GET /users?status=PENDING → GET /users/{id}
7. PATCH /users/{id}/status  { "status": "APPROVED" }
8. POST /auth/login          (student APPROVED)
9. POST /auth/refresh
10. POST /auth/logout
```
...
Invoke-RestMethod -Method Post -Uri "http://localhost:8080/api/v1/auth/register" `
  -ContentType "application/json" `
  -Body '{"email":"student.internal.qa@gmail.com","password":"Student@qa123","confirmPassword":"Student@qa123"}'
```
