...
| `EXTERNAL_JUDGE_NOT_ALLOWED_IN_PRELIM` | Gán Judge `EXTERNAL` (guest) vào Track sơ loại | People → assign guest → track |
| `INVALID_ASSIGNMENT_TYPE` (CK) | Gán INTERNAL làm `FINAL_EXTERNAL`, hoặc EXTERNAL làm `HEAD` CK | GĐ4 assign judges |
...
**Rule nhân sự:** sơ loại chỉ Judge **INTERNAL** (`HEAD` + `NORMAL`). Guest EXTERNAL chỉ trên CK (`FINAL_EXTERNAL`). Seed pool: `judge1–4`, `mentor1–3`, `guestjudge`/`guestjudge2–3`.
...
| `WILDCARD_OFF_VIOLATION` | Tắt Wildcard trong Setting → gọi API/UI cấp vé Wildcard | `seal-gd4-wildcard-disabled` |
