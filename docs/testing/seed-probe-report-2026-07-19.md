# Seed API probe report — 2026-07-19

**Result:** 28/29 passed

| Slug | Status | Reason | Detail |
|------|--------|--------|--------|
| `seal-e2e-2026` | PASS |  |  |
| `seal-fall-2025-finished` | PASS |  |  |
| `seal-gd3-prelim-open` | PASS |  |  |
| `seal-gd4-advance-ready` | PASS |  | chain ok: 3 events, 8 teams, 8 ranked |
| `seal-gd4-tiebreak-submission-time` | PASS |  | chain ok: 3 events, 3 teams, 3 ranked |
| `seal-gd4-tiebreak-manual` | PASS |  | chain ok: 3 events, 2 teams, 2 ranked |
| `seal-gd4-wildcard-gap` | PASS |  | chain ok: 3 events, 8 teams, 8 ranked |
| `seal-gd5-final-active` | PASS |  | chain ok: 3 events, 4 teams, 4 ranked |
| `seal-gd6-pending-confirm` | PASS |  | chain ok: 3 events, 3 teams, 3 ranked |
| `account:unverified-student` | PASS |  |  |
| `account:pending-mentor` | PASS |  |  |
| `account:pending-judge` | PASS |  |  |
| `account:rejected-judge` | PASS |  |  |
| `account:approved-unverified-mentor` | PASS |  |  |
| `neg:team-on-archived` | PASS |  |  |
| `neg:user-in-another-team` | PASS |  |  |
| `neg:registration-elsewhere` | PASS |  |  |
| `neg:invalid-repo-platform` | PASS |  |  |
| `neg:scoring-not-open` | PASS |  |  |
| `neg:archived-mutation` | PASS |  |  |
| `neg:oauth-token-invalid` | PASS |  |  |
| `neg:duplicate-email` | FAIL | expected one of [ACCOUNT_DUPLICATE_EMAIL], got VALIDATION_FAILED — Yêu cầu không hợp lệ |  |
| `neg:invalid-credentials` | PASS |  |  |
| `neg:forbidden-wrong-role` | PASS |  |  |
| `neg:scoring-locked` | PASS |  |  |
| `neg:email-verification-token-invalid` | PASS |  |  |
| `neg:journey-idor` | PASS |  |  |
| `neg:queue-cross-hackathon` | PASS |  |  |
| `neg:student-score-forbidden` | PASS |  |  |
