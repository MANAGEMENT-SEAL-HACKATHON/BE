# MF-01 GĐ1 — Test happy path & dữ liệu mẫu

**Mục đích:** Checklist thực thi test API theo mainflow GĐ1 (7 bước), JSON copy-paste, biến Postman, và map seed dev.
...
| API prefix | `/api/v1` |
| Profile | `spring.profiles.active=dev` → chạy `Gd1DataSeeder` |
| Auth | **Stub** — `@CoordinatorOnly` coi user **id=1** là Coordinator (không cần JWT) |
| Coordinator seed | `coord@fpt.edu.vn` — phải là **user id=1** trên DB trống |
| Response | Thành công: `{ "success": true, "data": ... }` — Lỗi: `error.code`, `errors[]` |
