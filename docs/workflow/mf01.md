...
Lớp 2	Không chồng / trùng	Tối đa 1 milestone/type; không overlap cùng type; OTHER/PRESENTATION không chồng milestone	BLOCK CỨNG 422
Lớp 3	Thứ tự giai đoạn	WORKSHOP → KICKOFF → AWARDS (`effectiveEnd` trước `startsAt` sau)	BLOCK CỨNG 422
...
AWARDS	Lễ trao giải Chung kết (cuối tháng 11)	Lễ trao giải Chung kết 12/4 17:30–19:00
7.5 Ràng buộc nghiệp vụ (v3.3 — không ràng buộc chéo Event↔Round)
Ràng buộc	Tầng	Hành động khi vi phạm
...
Lớp 1 (AWARDS): date == event_end; effectiveEnd ≤ event_end	App	422 EVENT_OUT_OF_HACKATHON
Lớp 1 (PRESENTATION / OTHER): date ∈ [event_start, event_end]	App	422 EVENT_OUT_OF_HACKATHON
Lớp 2: Không 2 milestone/type song song (1 milestone / type / hackathon; milestone = WORKSHOP, KICKOFF, AWARDS)	App	422 EVENT_MILESTONE_DUPLICATE
Lớp 3: WORKSHOP→KICKOFF→AWARDS (endsAt nối tiếp)	App	422 EVENT_ORDER_VIOLATION
Lớp 3d: KICKOFF trong [event_start, event_start+1d]	App	Warn mềm EVENT_ORDER_INVALID
OTHER/PRESENTATION ↔ milestone không chồng giờ	App	422 EVENT_CONFLICTS_WITH_MILESTONE
Có round CK → bắt buộc AWARDS (gate readiness)	App (Gate)	422 EVENT_AWARDS_MISSING (chỉ ONGOING gate — không block delete)
DELETE milestone → revalidate round.examAt	App	422 ROUND_EXAM_* (không còn EVENT_AWARDS_MISSING khi delete)
round.examAt > KICKOFF.endsAt	App	422 ROUND_EXAM_BEFORE_KICKOFF
round.examAt ∈ [event_start, event_end]	App	422 EVENT_OUT_OF_HACKATHON
CK.examAt < AWARDS.startsAt (nếu AWARDS đã tạo)	App	422 ROUND_EXAM_OUTSIDE_AWARDS
