...
/**
 * Seed GĐ4 — published + 6 ADVANCED + guest judge, CK không có criteria.
 *
 * <p>PATCH activate → 422 {@code ROUND_NO_CRITERIA}
 */
@Slf4j
@Component
@Profile("dev")
@RequiredArgsConstructor
public class Gd4CkNoCriteriaDataSeeder {
    ...
    @Value("${app.seed.gd4.ck-no-criteria.enabled:true}")
    private boolean enabled;

    @Transactional
    public void ensureSeed() {
        if (!enabled) {
            log.info("[Gd4CkNoCriteriaDataSeeder] Tắt (app.seed.gd4.ck-no-criteria.enabled=false)");
            return;
        }
        ...
        HackathonDevSeedHelper.HackathonStructure structure = seedHelper.ensureHackathonStructure(
                ...
                "SEAL GĐ4 — CK no criteria",
                HackathonStatus.ONGOING,
                "Seed GĐ4 — guest judge CK nhưng không criteria → ROUND_NO_CRITERIA",
                prelimState,
        ...
        for (int i = 0; i < Gd4CkActivateReadySeedConstants.TEAM_NAMES.length; i++) {
            ...
            User leader = seedHelper.upsertStudent(
                    Gd4CkNoCriteriaSeedConstants.studentEmail(idx),
                    Gd4CkNoCriteriaSeedConstants.studentDisplayName(idx),
                    hcm);
        ...
        seedHelper.clearFinalRoundCriteria(finalRound);

        log.info("""
                [Gd4CkNoCriteriaDataSeeder] slug={} finalRoundId={}
                  guest judge assigned, 0 criteria CK — PATCH activate → ROUND_NO_CRITERIA
                """,
                Gd4CkNoCriteriaSeedConstants.SLUG_GD4_CK_NO_CRITERIA,
                finalRound.getId());
    }
    ...
    public void repairForFeTesting() {
        ...
        hackathonRepository.findBySlug(Gd4CkNoCriteriaSeedConstants.SLUG_GD4_CK_NO_CRITERIA).ifPresent(h -> {
            ...
            if (needsRepair(h, prelim, finalRound)) {
                ...
                reapplyState(h, prelim, finalRound);
            }
            seedHelper.syncHackathonCalendarFromDates(
                    Gd4CkNoCriteriaSeedConstants.SLUG_GD4_CK_NO_CRITERIA,
                    seedHelper.computeGd4AdvanceReadyDates());
    ...
    private void reapplyState(Hackathon hackathon, Round prelim, Round finalRound) {
        for (int advancedIndex : Gd4CkActivateReadySeedConstants.ADVANCED_TEAM_INDICES) {
    ...
    public void resetAndSeed() {
        devSeedCleanup.purgeIfPresent(Gd4CkNoCriteriaSeedConstants.SLUG_GD4_CK_NO_CRITERIA);
        ensureSeed();
    }
