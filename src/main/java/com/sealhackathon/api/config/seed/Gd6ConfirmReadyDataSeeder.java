...
/**
 * Seed GĐ6 — đủ FIRST/SECOND/THIRD, sẵn sàng confirm.
 *
...
public class Gd6ConfirmReadyDataSeeder {
    ...
    @Value("${app.seed.gd6.confirm-ready.enabled:true}")
    private boolean enabled;

    @Transactional
    public void ensureSeed() {
        if (!enabled) {
            log.info("[Gd6ConfirmReadyDataSeeder] Tắt (app.seed.gd6.confirm-ready.enabled=false)");
            return;
        }
        ...
        HackathonDevSeedHelper.HackathonStructure structure = seedHelper.ensureHackathonStructure(
                ...
                "SEAL GĐ6 — Confirm ready",
                HackathonStatus.PENDING_CONFIRM,
                "Seed GĐ6 — 3 giải đã trao, PATCH /confirm → FINISHED",
        ...
        log.info("""
                [Gd6ConfirmReadyDataSeeder] slug={} hackathonId={}
                  3 prizes — PATCH /confirm → FINISHED
                """,
                Gd6ConfirmReadySeedConstants.SLUG_GD6_CONFIRM_READY,
                hackathon.getId());
    }
    ...
    public void repairForFeTesting() {
        ...
        hackathonRepository.findBySlug(Gd6ConfirmReadySeedConstants.SLUG_GD6_CONFIRM_READY).ifPresent(h -> {
            Round prelim = loadPrelim(h.getId());
            Round finalRound = loadFinal(h.getId());
            if (needsRepair(h, prelim, finalRound)) {
                seedHelper.repairHackathonForGd6Retest(h, prelim, finalRound);
                reapplyPrizes(h, prelim, finalRound);
            }
            seedHelper.syncHackathonCalendarFromDates(
                    Gd6ConfirmReadySeedConstants.SLUG_GD6_CONFIRM_READY,
                    seedHelper.computeGd6PendingConfirmDates());
    ...
    private void reapplyPrizes(Hackathon hackathon, Round prelim, Round finalRound) {
        User coordinator = seedHelper.requireCoordinator();
        for (int i = 0; i < Gd6ConfirmReadySeedConstants.TEAM_NAMES.length; i++) {
            teamRepository.findByHackathon_IdAndTeamNameIgnoreCase(
                            hackathon.getId(), Gd6ConfirmReadySeedConstants.TEAM_NAMES[i])
                    .ifPresent(team -> {
                        if (team.getTeamName().contains("FIRST")) {
                            seedHelper.ensureFirstPrize(hackathon, finalRound, team, coordinator);
                        } else if (team.getTeamName().contains("SECOND")) {
                            seedHelper.ensureSecondPrize(hackathon, finalRound, team, coordinator);
                        } else if (team.getTeamName().contains("THIRD")) {
    ...
    public void resetAndSeed() {
        devSeedCleanup.purgeIfPresent(Gd6ConfirmReadySeedConstants.SLUG_GD6_CONFIRM_READY);
        ensureSeed();
    }
    ...
    private List<Team> seedTeamsAndScores(
        ...
        for (int i = 0; i < Gd6ConfirmReadySeedConstants.TEAM_NAMES.length; i++) {
            ...
            User leader = seedHelper.upsertStudent(
                    Gd6ConfirmReadySeedConstants.studentEmail(idx),
                    Gd6ConfirmReadySeedConstants.studentDisplayName(idx),
                    chapters[i]);
            seedHelper.registerStudent(hackathon, leader);
            Team team = seedHelper.ensureActiveTeam(
                    hackathon, Gd6ConfirmReadySeedConstants.TEAM_NAMES[i], leader, chapters[i], now);
            ...
            Submission sub = seedHelper.ensureFinalSubmission(
                    ...
                    "https://github.com/seal-warriors/gd6r-team%02d".formatted(idx));
            float score = Gd6ConfirmReadySeedConstants.TEAM_SCORES[i];
